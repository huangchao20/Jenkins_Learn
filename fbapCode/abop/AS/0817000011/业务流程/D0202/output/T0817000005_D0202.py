...
def TD0202_STEP2_NODE1(__REQ__,__RSP__,__SND__,__RCV__):
    try:
        ...
        _Arg0_ = "select  protocolno from afa_memy_protocolinfo  where   protocolstatus=? and productmaincode=?  and productcode=? and clienttype=?";
        ACMP_Builtin_LoggerVar(4,"A0",_Arg0_);
        _Arg1_ = ["A2","OP","0817000005","0"];
        ACMP_Builtin_LoggerVar(4,"A1",_Arg1_);
        _Arg2_ = 0;
...
def TD0202_STEP2_NODE2(__REQ__,__RSP__,__SND__,__RCV__):
    try:
        AFALoggerInfor("功能 检验协议");

        _Arg0_ = __REQ__["AgreeMentInfo"][0][0]==__REQ__["__BODY__"]["AgreementID"];
        ACMP_Builtin_LoggerVar(4,"A0",_Arg0_);
...
def TD0202_STEP2_NODE8(__REQ__,__RSP__,__SND__,__RCV__):
    try:
        ...
        _Arg0_ = __REQ__;
        ACMP_Builtin_LoggerVar(4,"A0","__REQ__");
        _Arg1_ = [	["Count",0],	["Amount",0.0],	["LineNo",0]];
        ACMP_Builtin_LoggerVar(4,"A1",_Arg1_);
...
def TD0202_STEP3_NODE9(__REQ__,__RSP__,__SND__,__RCV__):
    try:
        ...
        _Arg0_ = (len(__REQ__["OneInfoList"]) == 3) and(len(__REQ__["OneInfoList"][0])<=20 and __REQ__["OneInfoList"][0].strip() != "") and (__REQ__["OneInfoList"][2].strip() != "") and ((len(__REQ__["OneInfoList"][2].split('.'))==1) or (len(__REQ__["OneInfoList"][2].split('.'))==2 and len(__REQ__["OneInfoList"][2].split('.')[1])<=2));
        ACMP_Builtin_LoggerVar(4,"A0",_Arg0_);
...
def TD0202_STEP3_NODE11(__REQ__,__RSP__,__SND__,__RCV__):
    try:
        AFALoggerInfor("功能 重复信息校验");

        _Arg0_ = "select USERID from BPDM_OP_LZGASPAYMENT where USERID = ?  and CREATEDATE = ?  and RecordStatus = ?  ";
        ACMP_Builtin_LoggerVar(4,"A0",_Arg0_);
        _Arg1_ = [__REQ__["OneInfoList"][0],__REQ__["CurrentDate"],"S"];
        ACMP_Builtin_LoggerVar(4,"A1",_Arg1_);
        _Arg2_ = 0;
        ACMP_Builtin_LoggerVar(4,"A2",_Arg2_);
...
def TD0202_STEP3_NODE12(__REQ__,__RSP__,__SND__,__RCV__):
    try:
        ...
        _Arg1_ = "OP30";
        ACMP_Builtin_LoggerVar(4,"A1",_Arg1_);
        _Arg2_ = "第"+str(__REQ__["LineNo"])+"行数据重复，请检查！";
        ACMP_Builtin_LoggerVar(4,"A2",_Arg2_);
...
def TD0202_STEP3_NODE13(__REQ__,__RSP__,__SND__,__RCV__):
    try:
        ...
        _Arg0_ = __REQ__;
        ACMP_Builtin_LoggerVar(4,"A0","__REQ__");
        _Arg1_ = [	["Count",__REQ__["Count"]+1],	["Amount","%.2f"%(float(__REQ__["Amount"])+float(__REQ__["OneInfoList"][2]))]];
        ACMP_Builtin_LoggerVar(4,"A1",_Arg1_);
...
def TD0202_STEP4_NODE3(__REQ__,__RSP__,__SND__,__RCV__):
    try:
        ...
        ACMP_Builtin_LoggerVar(4,"A0",_Arg0_);
        _Arg1_ = [ ["ExcuteStatus","21"], ["Status","1"], ["ErroCode","0000"], ["ErroMsg",""] ];
        ACMP_Builtin_LoggerVar(4,"A1",_Arg1_);
...
def MD0202_ENTRY(__REQ__,__RSP__):
    AFALoggerInfor("开始交易 D0202:水费代扣文件校验");

    __SND__ = {};
    __RCV__ = {};

    __REQ__["__TRADENAME__"] = "水费代扣文件校验";
