...
def TD0205_STEP4_NODE12(__REQ__,__RSP__,__SND__,__RCV__):
    try:
        ...
        _Arg0_ = "select  accno, accname, documenttype, documentno  from afa_memy_protocolinfo  where protocolstatus=? and productcode=?  and  productmaincode=? and signinfo=? and PROTOCOLNO=?";
        ACMP_Builtin_LoggerVar(4,"A0",_Arg0_);
        _Arg1_ = ["A2",__REQ__["__BODY__"]["AppID"],__REQ__["__BODY__"]["AppType"],__REQ__["Record"][1],__REQ__["__BODY__"]["AGREEMENTID"]];
        ACMP_Builtin_LoggerVar(4,"A1",_Arg1_);
        _Arg2_ = 0;
        ACMP_Builtin_LoggerVar(4,"A2",_Arg2_);
...
def TD0205_STEP4_NODE16(__REQ__,__RSP__,__SND__,__RCV__):
    try:
        ...
        _Arg0_ = __REQ__;
        ACMP_Builtin_LoggerVar(4,"A0","__REQ__");
        _Arg1_ = [	["headInfor",__REQ__["HostBatchFileNew"]+"|#|"+				 "1"+"|#|"+				 "0"+"|#|"+				 "0"+"|#|"+				 str(__REQ__["TotalAmount"])+"|#|"+				 str(__REQ__["TotalMoney"])+"|#|"+				 "1104"+"|#|"+				 "01"+"|#|"+				 "12345678"+"|#|"+				 "200"+"|#|"+				 "南部校园一卡通批量代扣"+"|#|"+				 __REQ__["AcctInfo"][0][4]	]];
        ACMP_Builtin_LoggerVar(4,"A1",_Arg1_);
...
def TD0205_STEP5_NODE14(__REQ__,__RSP__,__SND__,__RCV__):
    try:
        ...
        _Arg0_ = "select  accno, accname, documenttype, documentno  from afa_memy_protocolinfo  where protocolstatus=? and productcode=?  and  productmaincode=? and signinfo=? and  ENTERPROTOCOLNO=?";
        ACMP_Builtin_LoggerVar(4,"A0",_Arg0_);
        _Arg1_ = ["A2",__REQ__["__BODY__"]["AppID"],__REQ__["__BODY__"]["AppType"],__REQ__["Record"][1],__REQ__["__BODY__"]["AgreementID"],];
        ACMP_Builtin_LoggerVar(4,"A1",_Arg1_);
        _Arg2_ = 0;
...
def MD0205_ENTRY(__REQ__,__RSP__):
    AFALoggerInfor("开始交易 D0205:代扣主机文件创建");

    __SND__ = {};
    __RCV__ = {};

    __REQ__["__TRADENAME__"] = "代扣主机文件创建";
