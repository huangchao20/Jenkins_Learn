...
def crow_Fi(n):
    url = 'https://search.jd.com/Search?keyword=手机&enc=utf-8&qrst=1&rt=1&stop=1&vt=2&cid2=653&cid3=655&page=%s' % (str(2*n - 1))
    ...
    # datas = html1.xpath('//li[contains(@class, "gl-item")]')
    print(datas)
    with open('JD_phone.csv', 'w', newline='', encoding='utf-8') as fs:
        write = csv.writer(fs)
        for data in datas:
